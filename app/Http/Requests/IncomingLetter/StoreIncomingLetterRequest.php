...
/**
 * Validates incoming letter creation payload and PDF upload.
 *
 * Business rules:
 * - department_id must be active and not soft-deleted.
 * - letter_number unique among non-deleted rows (external numbers allowed).
 *
 * Related modules: IncomingLetterPolicy, StoreIncomingLetterService.
 */
class StoreIncomingLetterRequest extends FormRequest
{
    /**
     * Requires create permission on IncomingLetter model.
     */
    public function authorize(): bool
    {
        return $this->user()?->can('create', \App\Models\IncomingLetter::class) ?? false;
    }
    ...
    public function rules(): array
    {
        return [
            'letter_number' => [
                ...
                'string',
                'max:255',
                Rule::unique('incoming_letters', 'letter_number')->whereNull('deleted_at'),
            ],
