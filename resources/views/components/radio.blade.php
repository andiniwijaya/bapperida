...
<div class="flex items-start">
    <div class="flex items-center h-5">
        <input type="radio" id="{{ $controlId }}" name="{{ $name }}" value="{{ $value }}"
            ...
            class="ds-radio disabled:cursor-not-allowed disabled:opacity-50 {{ $error ? 'is-error' : '' }} {{ $class }}"
            {{ $attributes }} />
    </div>

    @if ($label)
        <div class="ml-3 text-sm">
            <label for="{{ $controlId }}" class="cursor-pointer font-medium ds-body">
                {{ $label }}
                @if ($required)
